<?php 
    ...
    $stmt = $conn->prepare(
        ...
        WHERE (p.STATUS_PRODUTO = 6) AND p.ID = ?;
    ...
    if (isset($_POST['aprovar'])) {
        ...
        $stmt_update_recusa = $conn->prepare("UPDATE produto SET STATUS_PRODUTO = 2 WHERE ID = ? AND ID_USUARIO = ?");
        ...
        $_SESSION['leilao-restaurado'] = "<p><i class='material-icons'>check_circle</i> Restauração de leilão feita com sucesso</p>";
            
        header('Location: lista-leilao-suspenso.php');
        exit;
    }
...
<body>
    ...
    <main>
        ...
        <section class="main-content">
            <div class="categoria-row categoria-row-solicitacao"></div>
            <form action="<?=$_SERVER['PHP_SELF']?>?id=<?=$id_produto?>" method="POST" class="iniciando-leilao" style="padding-right: 57px;" enctype="multipart/form-data">
                ...
                <div class="main-div-2">
                    ...
                    <div class="aprova-recusa">
                        <button type="submit" class="aprovar" style="visibility: hidden;">Aprovar</button>
                        <a href="#" id="btn-restaurar" name="aprovado" class="aprovar" 
                        style="background-color: #ffd980; color: #5e2a1e; padding: 10px 20px;">
                        Restaurar
                        </a>
                    </div>

                    <div id="modalLogout1" class="modal">
                        <div class="modal-content">
                            <p>Tem certeza que deseja restaurar esse leilão?</p>
                            <button id="confirmarSair1" class="confirmar-sair1" type="submit" name="aprovar">Sim</button>
                            <button id="cancelarSair1" class="cancelar-sair1">Cancelar</button>
                        </div>
                    </div>

                    <script>
                        // Abrir modal
                        document.getElementById('btn-restaurar').addEventListener('click', function(e) {
                            e.preventDefault();
                            document.getElementById('modalLogout1').style.display = 'flex';
                        });

                        // Cancelar modal
                        document.getElementById('cancelarSair1').addEventListener('click', function() {
                            document.getElementById('modalLogout1').style.display = 'none';
                        });
                    </script>
                </div>
            </form>
