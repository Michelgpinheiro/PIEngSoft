<?php 
    ...
    if (!empty($_POST['motivo-recusa']) && isset($_POST['recusar'])) {
        $suspensao = $_POST['motivo-recusa'];

        $stmt_recusa = $conn->prepare("UPDATE produto SET MOTIVO_SUSPENSAO = ? WHERE ID = ? AND ID_USUARIO = ?");
        $stmt_recusa->bind_param("sii", $suspensao, $id_produto, $user_id);
        $stmt_recusa->execute();
        $stmt_recusa->close();

        $stmt_update_recusa = $conn->prepare("UPDATE produto SET STATUS_PRODUTO = 6 WHERE ID = ? AND ID_USUARIO = ?");
        ...
        $_SESSION['leilao-suspenso'] = "<p><i class='material-icons'>check_circle</i> Suspensão de leilão feita com sucesso</p>";
            ...
            <form action="<?=$_SERVER['PHP_SELF']?>?id=<?=$id_produto?>" method="POST" class="iniciando-leilao" style="padding-right: 57px;" enctype="multipart/form-data">
                ...
                <div class="main-div-2">
                    ...
                    <div class="aprova-recusa">
                        ...
                        <a href="#" id="btn-dar-lance" class="aprovar" style="background-color: #ffd980; color: #5e2a1e; padding: 10px 20px;">Suspender</a>
                    </div>
                    ...
                    <div class="popup-container pop-container-recusa" id="popup-lance" style="position: absolute; bottom: -200px;">
                        ...
                        <span id="close-popup" class="close-popup">&times;</span>
                            ...
                            <button id="enviar-lance" name="recusar" type="submit" >Suspender</button>
                        </div>
                    </div>
                </div>
            </form>
