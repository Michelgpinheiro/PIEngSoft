<?php 
    ...
    if (isset($_POST['aprovado'])) {
        ...
        $stmt_verificado = $conn->prepare("UPDATE leilao SET VERIFICADO = 1 WHERE ID_PRODUTO = ? AND ID_USUARIO = ?");
        $stmt_verificado->bind_param("ii", $_SESSION['id-produto'], $_SESSION['user-id']);
        $stmt_verificado->execute();
        $stmt_verificado->close();

        $stmt_update_aprovado = $conn->prepare("UPDATE produto SET STATUS_PRODUTO = 2 WHERE ID = ? AND ID_USUARIO = ?");
        $stmt_update_aprovado->bind_param("ii", $_SESSION['id-produto'], $_SESSION['user-id']);
        $stmt_update_aprovado->execute();
        $stmt_update_aprovado->close();

        unset($_SESSION['id-produto'], $_SESSION['user-id']);

        $_SESSION['solicitacao-aprovada'] = "<p><i class='material-icons'>check_circle</i> Solicitação aprovada com sucesso</p>";
    ...
    $stmt = $conn->prepare(
        ...
        WHERE p.STATUS_PRODUTO = 5 AND p.ID = ?;
    ...
    if (!empty($_POST['motivo-recusa']) && isset($_POST['recusar'])) {
        $recusa = $_POST['motivo-recusa'];

        $stmt_recusa = $conn->prepare("UPDATE leilao SET MOTIVO_RECUSA = ?, 
        VERIFICADO = 1 WHERE ID_PRODUTO = ? AND ID_USUARIO = ?");
        $stmt_recusa->bind_param("sii", $recusa, $id_produto, $user_id);
        $stmt_recusa->execute();
        $stmt_recusa->close();

        $stmt_update_recusa = $conn->prepare("UPDATE produto SET STATUS_PRODUTO = 3 WHERE ID = ? AND ID_USUARIO = ?");
...
<body>
    ...
    <main>
        ...
        <section class="main-content">
            ...
            <form action="solicitacao-adm.php?id=<?=$id_produto?>" method="POST" class="iniciando-leilao" style="padding-right: 57px;" enctype="multipart/form-data">
                <h2>Autorização de Leilão</h2>
                <div class="main-div-1">
                    <div class="nome-imagens-pracas-reducao-valorincremento">
                        <a class="voltar" href="tela-solicitacao-adm.php">
                ...
                <div class="main-div-2">
                    ...
                    <div class="aprova-recusa">
                        <button type="submit" name="aprovado" class="aprovar">Aprovar</button>
                        <a href="#" id="btn-dar-lance" class="recusar">Recusar</a>
                    </div>

                    <div class="popup-container pop-container-recusa" id="popup-lance" style="position: absolute; bottom: -270px; right: -50px;">
                        ...
                        <span id="close-popup" class="close-popup">&times;</span>
                            ...
                            <button id="enviar-lance" name="recusar" type="submit" >Enviar</button>
                        </div>
                    </div>
                </div>
            </form>
